...
namespace MCForge
{
    ...
    public sealed class MapGenerator
    {
        ...
        public void GenerateTrees(Level map)
        {
            ...
            for (int x = 0; x < map.width; x += rn.Next(MinTrunkPadding, MaxTrunkPadding))
            {
                for (int y = 0; y < map.depth; y += rn.Next(MinTrunkPadding, MaxTrunkPadding))
                {
                    ...
                    if (map.GetTile(nx, ny, nz) == (byte)bGroundSurface)
                    {
